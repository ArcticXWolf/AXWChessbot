...
def print_state(board: chess.Board):
...
def test_engine():
    positions = [
        "1k1r4/pp3R2/6pp/4p3/2B3b1/4Q3/PPP2B2/2K5 b - - 2 3",
    ...
    for i in range(len(positions)):
        ...
        if str(board.san(move)) in str(solutions[i]):
            solved = solved + 1
            print(
                f"{i+1} OKAY: {board.san(move)} in {end_search - start_search :.2f} sec"
            )
        else:
            print(
                f"{i+1} ERRO: {board.san(move)} instead of {solutions[i]} in {end_search - start_search :.2f} sec"
            )

    end_whole = timer()

    print(
        f"Number correct solutions: {solved}/{len(positions)} in {end_whole - start_whole :.2f} sec"
    )
